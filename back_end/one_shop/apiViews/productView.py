...
class ProductDetailsView(APIView):
    ...
    def put(self, request, pk=None):
        ...
        multimedia_info_raw = request.data.get("multimediaInfo", "{}")

        if isinstance(multimedia_info_raw, str):
            multimedia_info = json.loads(multimedia_info_raw)
        else:
            multimedia_info = multimedia_info_raw

        if not isinstance(multimedia_info, dict):
            multimedia_info = {}
        ...
        if main_image_file:
            result = cloudinary.uploader.upload(
                main_image_file, folder="enouza/products"
            )
        ...
        additional_files = request.FILES.getlist("additionalImageFiles")

        if additional_files:
            image_urls = list(multimedia_info.get("image_urls", []))
        ...
        if color_files:
            ...
            for color_file in color_files:
                result = cloudinary.uploader.upload(
                    color_file, folder="enouza/products"
                )

                color_urls.append(result["secure_url"])

            data["colors"] = color_urls

        # ============================================================
        # MULTIMEDIA INFO
        # ============================================================

        data["multimediaInfo"] = multimedia_info
        ...
        serializer = ProductSerializer(product_to_update, data=data, partial=True)
